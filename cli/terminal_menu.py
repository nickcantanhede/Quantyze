...
Module Description
==================
This module contains the terminal menu and the small read-only artifact
...
class MenuConfig:
    ...
    @property
    def run_simulation(self) -> Callable[[RunArgs], None]:
...
def _build_run_args(
    ...
    """Return a small simulation configuration for the terminal menu.

    >>> args = _build_run_args(scenario='balanced', speed=2.0)
    >>> (args.scenario, args.speed)
    ('balanced', 2.0)
    """
    return make_run_args(data=data, scenario=scenario, speed=speed)
...
def _quick_ta_demo(config: MenuConfig) -> None:
    ...
    _run_simulation_menu(config, _build_run_args())
    _print_quick_ta_demo_followup()


def _run_simulation_menu(config: MenuConfig, args: RunArgs) -> None:
...
def _print_help_reference(config: MenuConfig) -> None:
    ...
    print("  - Run main.py in PyCharm or use python3 main.py")
    print("  - Open http://127.0.0.1:9000")
    print("  - Use the Simulate tab for the default synthetic demo")
    print("  - Use the Train tab for packaged sample, huge, or AAPL LOBSTER retraining")
    print("  - Use the Artifacts and Charts tabs to inspect saved state and outputs")
    print("Terminal fallback:")
    print('  Mac/Linux: python3 -c "import main; main.main(run_ui=False)"')
    print('  Windows:   py -3 -c "import main; main.main(run_ui=False)"')
...
def _simulation_menu(config: MenuConfig) -> None:
    ...
    while True:
        ...
        if choice == "1":
            ...
            _run_simulation_menu(config, _build_run_args(scenario=scenario, speed=speed))
        elif choice == "3":
            ...
            _run_simulation_menu(config, _build_run_args(data=data_path, speed=speed))
        elif choice == "4":
