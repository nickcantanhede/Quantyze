...
\section*{2. Datasets}
Quantyze uses three distinct \textbf{data-source roles}, and keeping those
roles separate is important for understanding the final submission:
\begin{itemize}
...
The final submission also includes a packaged dataset zip so that the TA does
not need to download anything externally before running the project.
...
waiting on a large dataset load.
...
LOBSTER data is fundamentally a \textbf{paired-file format}. Each sample
provides:
\begin{itemize}
...
Our code supports raw LOBSTER message and order-book CSVs. The TA points the
training flow at the \textbf{message file}, and our loader infers the paired
...
development to validate the real-data pipeline. In the final submission, we now
ship one official Apple 5-level sample pair inside the dataset zip using the
clearer extracted filenames:
\begin{itemize}
...
This packaged LOBSTER option is intentionally \textbf{optional}. It is useful
for demonstrating the real-data pipeline, but it is much larger and slower to
load than the short internal packaged dataset.
...
and metrics so that the TA can use the model path without retraining first.
...
The short TA path should start with \texttt{sample\_internal.csv}. The larger
internal dataset and the packaged LOBSTER pair are both slower-loading options
meant for a more thorough optional check.
...
synthetic data or describing external data abstractly, the final project now
separates three roles clearly: synthetic scenarios for replay, packaged
internal CSVs for fast reproducible retraining, and a packaged raw LOBSTER pair
for the real-data preprocessing path. This keeps the TA workflow self-contained
while still showing that the project can process real market data.
