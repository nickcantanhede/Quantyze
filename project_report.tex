...
\texttt{main.py}, with a terminal option for the same simulator and training
...
the terminal and UI runs look
for the packaged datasets and shipped baseline artifacts there.
...
\begin{itemize}
    ...
    \item the optional terminal fallback opened through
...
renders order-book depth and execution-price history after a simulation.

\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{ui_simulate_tab.png}
\caption{The \texttt{Simulate} tab of the Quantyze browser UI
(\texttt{http://127.0.0.1:9000}). The TA selects a synthetic scenario or CSV
dataset, sets replay speed, and chooses the active model overlay before
...
\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{ui_charts_tab.png}
\caption{The \texttt{Charts} tab after a simulation. The left panel shows the
current order-book depth as a horizontal bar chart (bids in green, asks in
red). The right panel plots execution prices over fill index, coloured by
side. The table below lists the 50 most recent fills with timestamp, side,
price, quantity, and maker order id.}
\label{fig:ui-charts}
\end{figure}
...
\texttt{training.py}, \texttt{api\_payloads.py}, and \texttt{web.py}, while
keeping \texttt{main.py} as a small entry point.
...
\begin{itemize}
    ...
    terminal run, saved-state workflow, metrics inspection, and
...
\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{ui_train_tab.png}
\caption{The \texttt{Train} tab. The TA selects a training dataset
(\texttt{sample\_internal.csv}, \texttt{huge\_internal.csv}, the packaged
LOBSTER pair, or a custom CSV path), then clicks \textbf{Start Training}.
The panel summarises the pipeline configuration before the run begins.}
\label{fig:ui-train}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{ui_artifacts_tab.png}
\caption{The \texttt{Artifacts} tab. It shows the active model overlay mode
and provenance, the file status of all saved checkpoints and metrics
artifacts, the baseline and latest training metrics side by side, and a
preview of the most recent execution log records.}
\label{fig:ui-artifacts}
\end{figure}

\subsection*{4.5 How the TA Should Interpret the Output}
The browser UI summaries and terminal fallback contain the same two classes of
...
The \texttt{Train} tab and the terminal fallback report validation accuracy,
...
owns classifier training, \texttt{web.py} serves the browser UI, and
\texttt{main.py} is now a small entry point.
...
simulator, and can be retrained from the browser UI or the terminal fallback.
